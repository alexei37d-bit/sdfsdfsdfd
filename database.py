...
class Database:
    ...
    def create_tables(self):
        """Создает таблицы пользователей и чеков"""
    ...
    def get_balance(self, user_id: int, currency: str):
        ...
        # Важно: currency.lower() должен совпадать с названием колонки (без пробелов!)
    ...
    def get_all_balances(self, user_id: int):
        ...
        if not row:
            return {}
        # Ключи должны быть БЕЗ пробелов, чтобы совпадать с логикой main.py
    ...
    def activate_check(self, check_id: str, activator_id: int):
        self.cursor.execute("UPDATE checks SET is_active = 0, activated_by = ? WHERE id = ?", (activator_id, check_id))
        self.conn.commit()
